...
import { Calendar, ArrowLeft, Share2, PenLine } from 'lucide-react'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const item = mockBlogs.find(b => b.slug === params.slug)
...
export default function BlogDetailPage({ params }: Props) {
  ...
  const related = mockBlogs.filter(b => b.slug !== params.slug).slice(0, 3)
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <article>
          ...
          <div className="flex items-center gap-4 text-sm text-gray-500 mb-8 pb-8 border-b border-white/10">
            ...
            {(item as any).author && (
              <span className="text-gray-400">{(item as any).author}</span>
            )}
          ...
          <div className="glass rounded-xl p-5 border border-violet-500/20 mb-8">
          ...
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: item.content || `<p>${item.summary}</p><p>完整内容正在整理中，敬请期待。</p>`
            }}
          />
        </article>
        ...
        {related.length > 0 && (
          <div className="mt-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {related.map(r => (
                <Link key={r.id} href={`/blog/${r.slug}`} className="group block glass rounded-xl p-4 border border-white/5 hover:border-violet-500/30 transition-all">
                  <span className="tag-badge text-xs mb-2 inline-block">{r.category?.name}</span>
                  <h4 className="text-sm font-medium text-white group-hover:text-violet-400 transition-colors line-clamp-2">{r.title}</h4>
        ...
        <div className="mt-12">
          <Link href="/blog" className="inline-flex items-center gap-2 text-sm text-gray-400 hover:text-white transition-colors">
